...
- **`main` and `gh-pages` hold different content, not copies of each other.** `main` is the source repo (`.tsx`, config, etc.) — `git push origin main` updates it and is what Vercel watches. `gh-pages` holds only the compiled build output (`index.html`, hashed `assets/*.js`/`*.css`) that `npm run deploy` overwrites each time — there's no source code readable on that branch. Pushing to `main` alone does **not** update GitHub Pages; a fix isn't live there until `npm run deploy` (or `/deploy_github_page`) runs again on top of it.
- `src/lib/player.ts`'s `requestMagicLink` builds `emailRedirectTo` from `window.location.origin + window.location.pathname` (not bare `origin`) specifically so this works on GitHub Pages, which serves the app from a subpath (`/claude_code_treasure_game_by_echo/`) rather than the domain root — bare `origin` drops that path and redirects land on a 404. Supabase's Redirect URLs allow-list match is exact string match, so whatever this expression produces (path *and* trailing slash included) must be listed byte-for-byte.
- Supabase **Authentication → URL Configuration → Redirect URLs** needs both production origins listed as **full URLs with a trailing slash**, matching what `origin + pathname` actually produces: `https://claude-code-treasure-game-by-echo.vercel.app/` and `https://echochen2023.github.io/claude_code_treasure_game_by_echo/`. A mismatch on the path or the trailing slash doesn't error — it silently falls back to the Site URL (`http://localhost:3000`), which is what makes this easy to misdiagnose.
