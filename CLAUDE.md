...
- Supabase **Authentication → URL Configuration → Redirect URLs** needs the GitHub Pages origin added too, separately from the Vercel one. **Must be the bare origin `https://echochen2023.github.io`, not the full page URL with the `/claude_code_treasure_game_by_echo/` path** — `emailRedirectTo: window.location.origin` never includes the path, only scheme+host+port, and Supabase's allow-list match is exact. Adding the path-included URL looks right but silently fails to match, so login falls back to the Site URL (`http://localhost:3000`) and breaks. This only bites GitHub Pages because it serves the app from a subpath (project page); Vercel's domain-root URL happens to equal its own origin so this trap doesn't show up there.
