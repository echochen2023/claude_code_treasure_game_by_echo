...
- GitHub 的 404 頁面(不是這個 app 的畫面):到 repo 的 **Settings → Pages** 確認 Source 是設成「Deploy from a branch」、分支選 `gh-pages`、資料夾選 `/ (root)`。第一次跑 `npm run deploy` 建立 `gh-pages` 分支後,這個設定通常要手動確認一次(不一定會自動生效)。
...
- 登入信的連結點了失敗:代表 Supabase 後台 **Authentication → URL Configuration → Redirect URLs** 沒有加入 `https://echochen2023.github.io/claude_code_treasure_game_by_echo/`。這個網址要額外加,跟 Vercel 那個網址是分開的兩筆設定,不會互相取代。
